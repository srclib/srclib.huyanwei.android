...
draw_home_button()
{
    ...
    if (getWidth() > getHeight()) {
        ...
        y -= 16; // move the house to the edge of the screen as it doesn't fill the texture.
    }
...
void drawFrontGrid(float rowOffset, float p)
{
    ...
    float rowHeight = 145.f;
    float yoff = 0.5f * h + 2.5f * rowHeight;
    if (w > h) {
        rowHeight -= 12.f;
        yoff = 0.47f * h + 1.0f * rowHeight;
    }

    int iconNum = (intRowOffset - 5) * g_Cols;
    ...
    for (row = -5; row < 15; row++) {
        ...
        for (col=0; col < g_Cols; col++) {
            ...
            if (iconNum >= 0) {
                float x = colWidth * col - (colWidth / 4);
                vpConstants->Position.x = x + 10.0f;
                ...
                if (state->selectedIconIndex == iconNum && !p) {
                    ...
                    vpConstants->ImgSize.x = SELECTION_TEXTURE_WIDTH_PX + 60;
                    vpConstants->ImgSize.y = SELECTION_TEXTURE_HEIGHT_PX + 60;
                    vpConstants->Position.y = y - (SELECTION_TEXTURE_HEIGHT_PX + 60 - ICON_TEXTURE_HEIGHT_PX) * 0.5f;
...
main(int launchID)
{
    ...
    g_PosMax = ((state->iconCount + g_Cols -1) / g_Cols) - g_Rows;
