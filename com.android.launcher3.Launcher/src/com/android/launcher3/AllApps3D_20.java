...
public class AllApps3D_20 extends RSSurfaceView
        implements AllAppsView, View.OnClickListener, View.OnLongClickListener, DragSource {
    private static final String TAG = "Launcher2.AllApps3D_20";
    ...
    public AllApps3D_20(Context context, AttributeSet attrs) {
        super(context, attrs);
        
        Log.d(TAG, "Launcher_MTK AllApps3D_20!!!");
    ...
    @SuppressWarnings({"UnusedDeclaration"})
    public AllApps3D_20(Context context, AttributeSet attrs, int defStyle) {
        this(context, attrs);
    }
            ...
            case KeyEvent.KEYCODE_DPAD_UP:
                if (mLastSelection == SELECTION_HOME) {
                    ...
                    if (currentPageRow > 0) {
						...
						newSelection = currentSelection - mColumnsPerPage < 0 ? newSelection
								: currentSelection - mColumnsPerPage;
                        sRollo.moveTo(newSelection / mColumnsPerPage);
    ...
    void initTouchState(int width, int height) {
        ...
        int cellHeight;//iconsSize / Defines.ROWS_PER_PAGE_PORTRAIT;
        int columnWidth;// TODO: Put this in a config file/define
        if(isPortrait){
        	columnWidth = width / mColumnsPerPage;
            if (DisplayMetrics.DENSITY_DEVICE <= 160) {
    			cellHeight = (height - sRollo.mParams.homeButtonHeight) / mRowsPerPage;
    		} else {
    			cellHeight = 145;
    		}
        }else {
        	columnWidth = (width - sRollo.mParams.homeButtonWidth) / mColumnsPerPage;
            if (DisplayMetrics.DENSITY_DEVICE <= 160) {
    			cellHeight = height / mRowsPerPage;
    		} else {
    			cellHeight = 145;
    		}
        }
...
*/
    ...
    */
    ...
    public static class RolloRS {
        ...
        public RolloRS(AllApps3D_20 allApps) {
            mAllApps = allApps;
        }

        public void init(Resources res, int width, int height) {
        	            
            Log.d(TAG, "Launcher_google RS init!!!");
            
            mRes = res;
            mWidth = width;
            mHeight = height;
			Defines.PORTRAIT = (width < height) ? 1 : 0;
            initProgramVertex();
            initProgramFragment();
            initProgramStore();
            initGl();
        ...
        private void initData() {
			...
			if (DisplayMetrics.DENSITY_DEVICE <= 160) {
				mParams.homeButtonTextureWidth = 84;
				mParams.homeButtonTextureHeight = 84;
			} else {
        ...
        private void initRs() {
			...
			if (Launcher.isQVGAMode()) {
                sb.setScript(mRes, R.raw.allapps20_ldpi);
			} else if (DisplayMetrics.DENSITY_DEVICE <= 160) {
				sb.setScript(mRes, R.raw.allapps20_160);
			} else {
				sb.setScript(mRes, R.raw.allapps20_3);
			}
            sb.setRoot(true);
