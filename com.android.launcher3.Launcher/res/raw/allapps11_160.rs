...
int lastFrame(int draw) {
    ...
    if (!ret)
    {
        ...
        if (g_TotalTime != 0 && g_FrameCount > 0)
        {
            float fps = 1000 / (g_TotalTime/g_FrameCount);
            if (state->zoomTarget == 0)
                debugF("Launcher2 fade out fps :", fps);
            else
                debugF("Launcher2 fade in fps :", fps);
...
int positionStrip(float row, float column, int isTop, float p, int isText)
{
    ...
    if (isTop) {
        matrixLoadTranslate(mat1, x, 0.5f, 0.f);
        matrixScale(mat1, scale, scale, 1.f);
    } else {
        matrixLoadTranslate(mat1, x, -0.40f, 0.f);
        matrixScale(mat1, scale, -scale, 1.f);
    }
    ...
    float soff_top = -(row * 0.80);
    float soff_bottom = -(row * 1.0);
    if (isTop) {
        matrixLoadScale(mat1, 1.f, -0.85f, 1.f);
        if (isText) {
            matrixScale(mat1, 1.f, 2.f, 1.f);
        }
        matrixTranslate(mat1, 0, soff_top - 0.95f + 0.20f, 0);
    } else {
        matrixLoadScale(mat1, 1.f, 0.85f, 1.f);
        if (isText) {
            matrixScale(mat1, 1.f, 2.f, 1.f);
        }
        matrixTranslate(mat1, 0, soff_bottom - 1.15f, 0);
    }
    vpLoadTextureMatrix(mat1);
    return -(soff_top + 0.3f) * 10.f;
}
...
draw_home_button()
{
    ...
    y -= 40; // move the house to the edge of the screen as it doesn't fill the texture.
...
void drawFrontGrid(float rowOffset, float p)
{
	...
	float iconWidth = 128.f;
    float rowHeight = colWidth + 7.f;
    ...
    float ymax = yoff + rowHeight - 10;
    float ymin = yoff - (3 * rowHeight) - 15 + 15;
    float gridTop = yoff - (3 * 3 / 5);
            ...
            if (iconNum >= 0) {
                ...
                if ((y >= ymin) && (y <= ymax)) {
                    ...
                    if (!p) {
                        ...
                        float px = ((x + iconWidth/2) - (getWidth() / 2)) / (getWidth() / 2);
                        float py = ((iconY + iconWidth/2) - (getHeight() / 2)) / (getWidth() / 2);
                        float d = (iconWidth/2) / (getWidth() / 2);
                        px *= p + 1;
                        py *= p + 1;
...
void drawTop(float rowOffset, float p)
{
    int row, col;
    int iconNum = 0;
    row = (int)(rowOffset+1) - 4;
    if(row < 0) row = 0;
    iconNum = row * 4;
    for (; row <= (int)(rowOffset+1); row++) {
        for (col=0; col < 4; col++) {
            if (iconNum >= state->iconCount) {
                return;
            }
